...
(function () {
  ...
  (function registerDiscussionNav() {
    ...
    function getPopupCloseBtn() {
      return query([
        '#app > .content-subpage-container.is-visible > div > div > div.header > span'
      ]);
    }
    ...
    // Close the popup (if open) and wait for the host to actually empty
    // out before firing the callback — SLS seems to no-op a card click
    // while a popup is already mounted, so we can't just open the next
    // one directly on top of the current one.
    function closePopup(callback, attemptsLeft = 10) {
      const host = getPopupHost();
      if (!host || !host.children.length) {
        callback();
        return;
      }
      const closeBtn = getPopupCloseBtn();
      if (closeBtn) closeBtn.click();

      if (attemptsLeft <= 0) { callback(); return; }
      setTimeout(() => {
        const h = getPopupHost();
        if (!h || !h.children.length) callback();
        else closePopup(callback, attemptsLeft - 1);
      }, 150);
    }
